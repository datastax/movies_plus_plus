      ...
      continueConversation: async ({ content }: any) => {
         ...
         const result = await streamUI({
            ...
            tools: {
               getMovies: {
                  ...
                  generate: async function* ({ query }) {
                     ...
                     const movies = await fetch(process.env.LANGFLOW_URL!, {
                        ...
                        .then((r) => r.json())
                        .then(
                           (d) => d.outputs[0].outputs[0].results.message.text
                        );
               ...
               showTrailer: {
                  description:
                     "When the user asks for a trailer, use this tool",
                  parameters: z.object({
                     movieName: z.string(),
                  }),
                  generate: async function* ({ movieName }) {
                     ...
                     yield (
                        <div className="flex items-center gap-4">
                           <IntegrationSpinner /> Getting more info about{" "}
                           {movieName}...
                        </div>
                     );
